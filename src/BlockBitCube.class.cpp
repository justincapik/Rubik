...
int		BlockBitCube::manhattanHeuristic(block_bits *blocks)
{
	...
	int		cornersvalue[8][24] =
	{

		{0, 4, 4, 1, 3, 1, 2, 2, 2, 1, 1, 3,  3, 1, 1, 2, 2, 2, 3, 3, 3, 2, 2, 2},
		{1, 1, 3, 0, 4, 4, 1, 3, 1, 2, 2, 2,  2, 2, 2, 3, 1, 1, 2, 2, 2, 3, 3, 3},
		{2, 2, 2, 1, 1, 3, 0, 4, 4, 1, 3, 1,  3, 3, 3, 2, 2, 2, 3, 1, 1, 2, 2, 2},
		{1, 3, 1, 2, 2, 2, 1, 1, 3, 0, 4, 4,  2, 2, 2, 3, 3, 3, 2, 2, 2, 3, 1, 1},

		{3, 1, 1, 2, 2, 2, 3, 3, 3, 2, 2, 2,  0, 4, 4, 1, 3, 1, 2, 2, 2, 1, 1, 3},
		{2, 2, 2, 3, 1, 1, 2, 2, 2, 3, 3, 3,  1, 1, 3, 0, 4, 4, 1, 3, 1, 2, 2, 2},
		{3, 3, 3, 2, 2, 2, 3, 1, 1, 2, 2, 2,  2, 2, 2, 1, 1, 3, 0, 4, 4, 1, 3, 1},
		{2, 2, 2, 3, 3, 3, 2, 2, 2, 3, 1, 1,  1, 3, 1, 2, 2, 2, 1, 1, 3, 0, 4, 4}
	};
	...
	for (int i = 0; i < 12; ++i)
	{
		value += sidesvalue[i][sides & 0x1f];
		sides >>= 5;
	}
	uint64_t corners = blocks->corners;
	for (int i = 7; i >= 0; --i)
	{
		//if (cornersvalue[i][corners & 0x1f] != 0)
		//	printf("number %d for %d = %d\n", i, corners & 0x1f, cornersvalue[i][corners & 0x1f]);
		value += cornersvalue[i][corners & 0x1f];
		corners >>= 5;
	}
