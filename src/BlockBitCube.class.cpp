...
bool	BlockBitCube::addcorner(int *corners, int c1, int c2, int c3)
{
	...
	for (int i = 0; i < 24; ++i)
	{
		if ((this->cornertable[i] & 0xfff) == (corner & 0xfff))
		{
			printf("(%02d) ", i);
			cout << "(" << bitset<4>(i) << ") ";
			*corners = (((*corners) << 4) & 0xfffffff0) | i;
			cout << "corners = " << bitset<32>(*corners) << endl;
			return (true);
		}
	}
	/*
...
bool	BlockBitCube::addsides(long *sides, int c1, int c2)
{
	...
	{
		if ((this->sidetable[i] & 0xfff) == (elem & 0xfff))
		{
			*sides = (((*sides) << 4) & 0xfffffffffffffff0) | i;
			cout << "sides = " << bitset<48>(*sides) << endl;
			return (true);
		}
	}
	printf("didn't work out\n");
...
block_bits		*BlockBitCube::bitToBlockCube(int *cube)
{
	...
	if (!this->addcorner(&(data->corners), CUBE3(YELLOW), CUBE7(ORANGE), CUBE5(GREEN))) // YOG corner 8
	...
	if (!this->addsides(&(data->sides), CUBE2(WHITE), CUBE6(RED))) // WR
		dprintf(2, "side 1 foire\n");
	if (!this->addsides(&(data->sides), CUBE4(WHITE), CUBE8(GREEN))) // WG
		dprintf(2, "side 2 foire\n");
	if (!this->addsides(&(data->sides), CUBE6(WHITE), CUBE2(ORANGE))) // WO
		dprintf(2, "side 3 foire\n");
	if (!this->addsides(&(data->sides), CUBE8(WHITE), CUBE4(BLUE))) // WB
		dprintf(2, "side 4 foire\n");
	if (!this->addsides(&(data->sides), CUBE4(RED), CUBE2(GREEN))) // RG
		dprintf(2, "side 5 foire\n");
	if (!this->addsides(&(data->sides), CUBE6(GREEN), CUBE4(ORANGE))) // GO
		dprintf(2, "side 6 foire\n");
	if (!this->addsides(&(data->sides), CUBE8(ORANGE), CUBE6(BLUE))) // OB
		dprintf(2, "side 7 foire\n");
	if (!this->addsides(&(data->sides), CUBE2(BLUE), CUBE8(RED))) // BR
		dprintf(2, "side 8 foire\n");
	if (!this->addsides(&(data->sides), CUBE2(RED), CUBE2(YELLOW))) // RY
		dprintf(2, "side 9 foire\n");
	if (!this->addsides(&(data->sides), CUBE4(GREEN), CUBE4(YELLOW))) // GY
		dprintf(2, "side 10 foire\n");
	if (!this->addsides(&(data->sides), CUBE6(ORANGE), CUBE6(YELLOW))) // OY
		dprintf(2, "side 11 foire\n");
	if (!this->addsides(&(data->sides), CUBE8(BLUE), CUBE8(YELLOW))) // BY
		dprintf(2, "side 12 foire\n");
...
int		*BlockBitCube::blockToBitCube(block_bits *blocks)
{
	...
	//          Y5
	//        6   7
	int		corners = blocks->corners;
	int		c;

	cout << "(" << bitset<4>(corners & 0xf) << ") ";
	printf("index %d in the corner table\n", corners & 0xf);
	c = this->cornertable[corners & 0xf];
